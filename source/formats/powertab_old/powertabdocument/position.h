...
public:
    // Default Constants
    static const uint8_t DEFAULT_POSITION; ///< Default value for the position
                                           ///< member variable
    static const uint8_t DEFAULT_BEAMING; ///< Default value for the beaming
                                          ///< member variable
    static const uint32_t DEFAULT_DATA; ///< Default value for the data member
                                        ///< variable
    static const uint8_t DEFAULT_DURATION_TYPE; ///< Default duration type

    // Position Constants
    static const uint32_t MIN_POSITION; ///< Minimum allowed value for the
                                        ///< position member variable
    static const uint32_t MAX_POSITION; ///< Maximum allowed value for the
                                        ///< position member variable

    // Irregular Grouping Constants
    static const uint8_t
        MIN_IRREGULAR_GROUPING_NOTES_PLAYED; ///< Minimum allowed value for the
                                             ///< irregular grouping notes
                                             ///< played value
    static const uint8_t
        MAX_IRREGULAR_GROUPING_NOTES_PLAYED; ///< Maximum allowed value for the
                                             ///< irregular grouping notes
                                             ///< played value
    static const uint8_t
        MIN_IRREGULAR_GROUPING_NOTES_PLAYED_OVER; ///< Minimum allowed value for
                                                  ///< the irregular grouping
                                                  ///< notes played over value
    static const uint8_t
        MAX_IRREGULAR_GROUPING_NOTES_PLAYED_OVER; ///< Maximum allowed value for
                                                  ///< the irregular grouping
                                                  ///< notes played over value

    // Volume Swell Constants
    static const uint8_t MAX_VOLUME_SWELL_DURATION; ///< Maximum allowed value
                                                    ///< for the volume swell
                                                    ///< duration

    // Tremolo Bar Constants
    static const uint8_t MAX_TREMOLO_BAR_DURATION; ///< Maximum allowed value
                                                   ///< for the tremolo bar
                                                   ///< duration
    static const uint8_t MAX_TREMOLO_BAR_PITCH; ///< Maximum allowed value for
                                                ///< the tremolo bar pitch (in
                                                ///< quarter steps)

    // Multibar Rest Constants
    static const uint8_t MIN_MULTIBAR_REST_MEASURE_COUNT; ///< Minimum allowed
                                                          ///< value for a
                                                          ///< multibar rest
                                                          ///< measure count
    static const uint8_t MAX_MULTIBAR_REST_MEASURE_COUNT; ///< Maximum allowed
                                                          ///< value for a
                                                          ///< multibar rest
                                                          ///< measure count

    static const size_t MAX_POSITION_COMPLEX_SYMBOLS =
        2; ///< Maximum allowed number of complex symbols per position object

    enum beamingFlags
    {
        irregularGroupingNotesPlayedOverMask =
            (uint16_t)0x07, ///< Mask used to get notes played over value
        irregularGroupingNotesPlayedMask =
            (uint16_t)0x78, ///< Mask used to get notes played value
        irregularGroupingTimingMask =
            (uint16_t)0x7f, ///< Mask used to filter out the irregular grouping
                            ///< timing values

        previousBeamDuration8th =
            (uint8_t)0x01, ///< Previous position in beam group is an 8th note
        previousBeamDuration16th =
            (uint8_t)0x02, ///< Previous position in beam group is a 16th note
        previousBeamDuration32nd =
            (uint8_t)0x03, ///< Previous position in beam group is a 32nd note
        previousBeamDuration64th =
            (uint8_t)0x04, ///< Previous position in beam group is a 64th note
        previousBeamDurationTypeMask =
            (uint16_t)0x0380, ///< Mask that filters out mututally exclusive
                              ///< beam duration flags

        beamStart =
            (uint16_t)0x400, ///< Note is the start of a group of beamed notes
        beamFractionalLeft =
            (uint16_t)0x800, ///< Note is half beam that draws left to right
        beamFractionalRight =
            (uint16_t)0x1000, ///< Note is half beam that draws right to left
        beamEnd =
            (uint16_t)0x2000, ///< Note is the end of a group of beamed notes

        beamingFlagsMask =
            (uint16_t)0x3f80 ///< Mask that filters out all possible beaming
                             ///< flags (previous beam duration + beam type)
    ...
    enum simpleFlags
    {
        ...
        dottedMask = (uint32_t)0x03, ///< Mask that filters out mutually
                                     ///< exclusive dotted flags
        ...
        vibratoMask = (uint32_t)0x18, ///< Mask that filters out mutually
                                      ///< exclusive vibrato flags

        arpeggioUp = (uint32_t)0x20, ///< Arpeggio up symbol
        arpeggioDown = (uint32_t)0x40, ///< Arpeggio down symbol
        arpeggioMask = (uint32_t)0x60, ///< Mask that filters out mutually
                                       ///< exclusive arpeggio flags

        pickStrokeUp = (uint32_t)0x80, ///< Pickstroke up symbol
        pickStrokeDown = (uint32_t)0x100, ///< Pickstroke down symbol
        pickStrokeMask = (uint32_t)0x180, ///< Mask that filters out mutually
                                          ///< exclusive pickstroke flags

        staccato = (uint32_t)0x200, ///< Notes are staccatoed

        marcato = (uint32_t)0x400, ///< Standard accent
        sforzando = (uint32_t)0x800, ///< Heavy accent
        accentMask = (uint32_t)0xc00, ///< Mask that filters out mutually
                                      ///< exclusive accent flags

        tremoloPicking = (uint32_t)0x1000, ///< Notes are plucked rapidly
        palmMuting =
            (uint32_t)0x2000, ///< Notes are muffled by the picking hand
        ...
        tripletFeel1st =
            (uint32_t)0x20000, ///< Indicates the notes should be played as if
                               ///< it were the 1st note in a triplet feel duet
        tripletFeel2nd =
            (uint32_t)0x40000, ///< Indicates the slash should be played as if
                               ///< it were the 2nd note in a triplet feel duet
        tripletFeelMask = (uint32_t)0x60000, ///< Mask that filters out mutually
                                             ///< exclusive triplet feel flags

        letRing = (uint32_t)0x80000, ///< Notes are held longer than the notated
                                     ///< duration
        fermata = (uint32_t)0x100000, ///< Fermata sign
        irregularGroupingStart =
            (uint32_t)0x200000, ///< Start of an irregular grouping
        irregularGroupingMiddle =
            (uint32_t)0x400000, ///< Middle of an irregular grouping
        irregularGroupingEnd =
            (uint32_t)0x800000, ///< End of an irregular grouping
        irregularGroupingFlagsMask =
            (uint32_t)0xe00000, ///< Masks that filters all possible irregular
                                ///< grouping types

        dataFlagsMask =
            (uint32_t)0xffffff, ///< Mask that filters out all valid data flags
        durationTypeMask =
            (uint32_t)0xff000000 ///< Mask used to retrieve the duration type
    ...
    // Member Variables
private:
    uint8_t m_position; ///< Zero-based index of the position within the system
                        ///< where the position is anchored
    uint16_t m_beaming; ///< Beaming and irregular grouping timing data
    uint32_t m_data; ///< Duration and simple symbol flags
    std::array<uint32_t, MAX_POSITION_COMPLEX_SYMBOLS>
        m_complexSymbolArray; ///< Array of complex symbols
