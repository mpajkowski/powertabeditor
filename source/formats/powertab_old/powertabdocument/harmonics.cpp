...
namespace
{
    std::map<uint8_t, uint8_t> harmonicsMap; ///< maps fret offsets to pitch
                                             ///< offsets

    /// initialize the mapping of frets to pitch offsets (counted in half-steps
    /// or frets) e.g. The natural harmonic at the 7th fret is an octave and a
    /// fifth - 19 frets - above the pitch of the open string
...
uint8_t getPitchOffset(uint8_t fretOffset)
{
    ...
    std::map<uint8_t, uint8_t>::const_iterator pitchOffset =
        harmonicsMap.find(fretOffset);
