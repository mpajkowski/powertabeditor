...
bool GuitarIn::operator==(const GuitarIn &guitarIn) const
{
    //------Last Checked------//
    // - Jan 25, 2005
    return ((m_system == guitarIn.m_system) && (m_staff == guitarIn.m_staff) &&
            (m_position == guitarIn.m_position) && (m_data == guitarIn.m_data));
}
...
bool GuitarIn::HasGuitarsSet(bool rhythmSlashes) const
{
    ...
    // - Jan 25, 2005
    uint8_t guitars =
        ((rhythmSlashes) ? GetRhythmSlashGuitars() : GetStaffGuitars());
    return (guitars != 0);
}
