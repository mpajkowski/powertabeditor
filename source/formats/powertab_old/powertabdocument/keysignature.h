...
public:
    ...
    enum flags
    {
        keyAccidentalsMask = (uint8_t)0x0f, ///< Bit mask used to retrieve the
                                            ///< key accidentals value
        show = (uint8_t)0x10, ///< Key signature is shown
        cancellation = (uint8_t)0x20, ///< Key signature is a cancellation
        keyTypeMask =
            (uint8_t)0x40 ///< Bit mask used to retrieve the key type value
    };

    enum keyTypes
    {
        majorKey =
            (uint8_t)0, ///< Key signature is a major key type (i.e. C major)
        minorKey =
            (uint8_t)1 ///< Key signature is a minor key type (i.e. A minor)
    };
    ...
    // Member Variables
private:
    uint8_t m_data; ///< Stores all data required by the key signature (see the
                    ///< flags enum for the meaning of the individual bits)
...
public:
    ...
    {
        if (set)
            SetFlag(cancellation);
        else
            ClearFlag(cancellation);
    }
