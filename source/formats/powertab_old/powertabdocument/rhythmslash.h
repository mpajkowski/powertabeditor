...
public:
    ...
    static const uint8_t MIN_SINGLE_NOTE_STRING_NUMBER; ///< Minimum allowed
                                                        ///< value for the
                                                        ///< single note string
                                                        ///< number
    static const uint8_t MAX_SINGLE_NOTE_STRING_NUMBER; ///< Maximum allowed
                                                        ///< value for the
                                                        ///< single note string
                                                        ///< number

    // Single Note Fret Number Constants
    static const uint8_t MIN_SINGLE_NOTE_FRET_NUMBER; ///< Minimum allowed value
                                                      ///< for the single note
                                                      ///< fret number
    static const uint8_t MAX_SINGLE_NOTE_FRET_NUMBER; ///< Maximum allowed value
                                                      ///< for the single note
                                                      ///< fret number

    enum beamingFlags
    {
        previousBeamDuration8th = (uint8_t)0x01, ///< Previous rhythm slash in
                                                 ///< beam group is an 8th note
        previousBeamDuration16th = (uint8_t)0x02, ///< Previous rhythm slash in
                                                  ///< beam group is a 16th note
        previousBeamDurationTypeMask =
            (uint8_t)0x03, ///< Mask that filters out mututally exclusive beam
                           ///< duration flags
        ...
        beamTypeMask = (uint8_t)0x1c, ///< Mask that filters out mutually
                                      ///< exclusive beam type flags

        beamingFlagsMask =
            (uint8_t)0x1f, ///< Mask that filters out all possible beaming flags
                           ///< (previous beam duration + beam type)
        ...
        tripletMask =
            (uint8_t)0xe0 ///< Mask that filters out all possible triplet flags
    };
    ...
    enum dataFlags
    {
        ...
        arpeggioMask = (uint32_t)0x600, ///< Mask that filters out mutually
                                        ///< exclusive arpeggio flags

        tripletFeel1st =
            (uint32_t)0x800, ///< Indicates the slash should be played as if it
                             ///< were the 1st note in a triplet feel duet
        tripletFeel2nd =
            (uint32_t)0x1000, ///< Indicates the slash should be played as if it
                              ///< were the 2nd note in a triplet feel duet
        tripletFeelMask = (uint32_t)0x1800, ///< Mask that filters out mutually
                                            ///< exclusive triplet feel flags

        marcato = (uint32_t)0x2000, ///< Standard accent
        sforzando = (uint32_t)0x4000, ///< Heavy accent
        accentMask = (uint32_t)0x6000, ///< Mask that filters out mutually
                                       ///< exclusive accent flags

        slideIntoFromAbove =
            (uint32_t)0x8000, ///< Slide into from a higher fret position
        slideIntoFromBelow =
            (uint32_t)0x10000, ///< Slide into from a lower fret position
        slideIntoMask = (uint32_t)0x18000, ///< Mask that filters out mutually
                                           ///< exclusive slide into flags

        slideOutOfDownwards =
            (uint32_t)0x20000, ///< Slide out of to a lower fret position
        slideOutOfUpwards =
            (uint32_t)0x40000, ///< Slide out of to a higher fret position
        slideOutOfMask = (uint32_t)0x60000, ///< Mask that filters out mutually
                                            ///< exclusive slide out flags

        dataFlagsMask =
            (uint32_t)0x7ffff, ///< Mask that filters out all valid data flags

        durationTypeMask =
            (uint32_t)0xe00000, ///< Mask used to retrieve the duration type
        stringNumberMask = (uint32_t)0x7000000, ///< Mask used to retrieve the
                                                ///< single note string number
        fretNumberMask = (uint32_t)0xf8000000, ///< Mask used to retrieve the
                                               ///< single note fret number
        singleNoteDataMask =
            (uint32_t)0xff000000 ///< Mask that filters out the single note data
...
private:
    uint8_t m_position; ///< Zero-based index of the position within the system
                        ///< where the rhythm slash is anchored
    uint8_t m_beaming; ///< Beaming and triplet data (top 3 bits = triplet
                       ///< flags, bottom 5 bits = beaming flags)
    uint32_t m_data; ///< Top byte = single note data, next 3 bits = duration
                     ///< type, remaining bits are flags
...
public:
    ...
    {
        if (!set)
            return (ClearDataFlag(rest));
        return (SetDataFlag(rest));
    }
    ...
    {
        if (!set)
            return (ClearDataFlag(tied));
        return (SetDataFlag(tied));
    }
    ...
    {
        if (!set)
            return (ClearDataFlag(muted));
        return (SetDataFlag(muted));
    }
