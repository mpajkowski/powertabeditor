...
// Purpose:         Constants and enumerations used by the PowerTabInputStream
// and PowerTabOutputStream classes Author:          Brad Larsen Modified by:
// Created:         Dec 19, 2004
// RCS-ID:
...
const uint16_t NEW_CLASS_TAG =
    0xffff; ///< Special tag indicating new CRuntimeClass
...
const uint16_t BIG_OBJECT_TAG =
    0x7fff; ///< 0x7fff indicates uint32_t object tag
...
const uint8_t BYTE_PLACEHOLDER =
    0xff; ///< Placeholder used when string length > 254
const uint16_t WORD_PLACEHOLDER =
    0xffff; ///< Placeholder used when string length > 65533
const uint16_t UNICODE_MARKER =
    0xfffe; ///< Marker used to indicate a Unicode string
const uint32_t DOUBLEWORD_PLACEHOLDER =
    0xffffffff; ///< Placeholder used when string length > 4294967295
...
#define _WX_DEFINE_POWERTABARRAY(T, name, base, classexp)                      \
    wxCOMPILE_TIME_ASSERT2(sizeof(T) <= sizeof(base::base_type),               \
                           TypeTooBigToBeStoredIn##base, name);                \
    ...
    public:                                                                    \
        ...
        {                                                                      \
            ...
            base::RemoveAt((size_t)iIndex);                                    \
                                                                               ...
                                                                               \
        void Sort(CMPFUNC##T fCmp)                                             \
        {                                                                      \
            base::Sort((CMPFUNC)fCmp);                                         \
        }                                                                      \
                                                                               \
...
#define WX_DEFINE_POWERTABARRAY(T, name)                                       \
    WX_DEFINE_USER_EXPORTED_POWERTABARRAY(T, name, wxBaseArrayPtrVoid,         \
