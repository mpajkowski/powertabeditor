...
const uint16_t PowerTabFileHeader::FILEVERSION_CURRENT =
    PowerTabFileHeader::Version_1_7;
...
const uint8_t PowerTabFileHeader::CONTENTTYPE_ALL =
    (CONTENTTYPE_GUITAR | CONTENTTYPE_BASS | CONTENTTYPE_PERCUSSION);
...
{
    return (
        (m_version == header.m_version) && (m_fileType == header.m_fileType) &&
        ...
        (m_songData.authorData.composer ==
         header.m_songData.authorData.composer) &&
        (m_songData.authorData.lyricist ==
         header.m_songData.authorData.lyricist) &&
...
bool PowerTabFileHeader::Serialize(PowerTabOutputStream &stream) const
{
    ...
    if (IsSong())
    {
        ...
        else if (m_songData.releaseType == RELEASETYPE_BOOTLEG)
        {
            ...
            stream << m_songData.bootlegData.month << m_songData.bootlegData.day
                   << m_songData.bootlegData.year;
            PTB_CHECK_THAT(stream.CheckState(), false);
        }
