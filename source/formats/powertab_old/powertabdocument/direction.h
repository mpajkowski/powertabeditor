...
public:
    ...
    // Symbol Constants
    static const uint8_t MAX_SYMBOLS; ///< Maximum allowed number of symbols in
                                      ///< the symbol array
    static const uint8_t NUM_SYMBOL_TYPES; ///< Number of direction symbol types

    // Repeat Number Constants
    static const uint8_t MIN_REPEAT_NUMBER; ///< Minimum allowed value for the
                                            ///< repeat number
    static const uint8_t MAX_REPEAT_NUMBER; ///< Maximum allowed value for the
                                            ///< repeat number
    ...
    enum flags
    {
        symbolTypeMask = (uint16_t)0xff00, ///< Mask used to retrieve the symbol
                                           ///< type (coda, segno, etc.)
        activeSymbolMask =
            (uint16_t)0xc0, ///< Mask used to retrieve the active symbol value
        repeatNumberMask =
            (uint16_t)0x3f ///< Mask used to retrieve the repeat number value
    };

    // Member Variables
private:
    uint8_t m_position; ///< Zero-based index of the position within the system
                        ///< where the direction is anchored
    std::vector<uint16_t> m_symbolArray; ///< Array of direction symbols
                                         ///< (symbol: top byte = symbol type,
                                         ///< next 2 bits = active symbol, next
                                         ///< 6 bits = repeat number)
