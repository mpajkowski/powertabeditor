...
struct CompareComplexSymbolType
{
    ...
    /// Extracts the type of the symbol given, and compares it to the supplied
    /// symbol type
...
/// found
template <size_t N>
uint32_t findComplexSymbol(const std::array<uint32_t, N> &symbolArray,
                           uint8_t type)
{
    typename std::array<uint32_t, N>::const_iterator symbol = std::find_if(
        symbolArray.begin(), symbolArray.end(), CompareComplexSymbolType(type));
...
{
    return symbolArray.size() - std::count(symbolArray.begin(),
                                           symbolArray.end(),
...
{
    std::fill(symbolArray.begin(), symbolArray.end(),
              static_cast<uint32_t>(notUsed));
}
...
void addComplexSymbol(std::array<uint32_t, N> &symbolArray, uint32_t symbolData)
{
    ...
    typename std::array<uint32_t, N>::iterator symbol = std::find_if(
        symbolArray.begin(), symbolArray.end(), CompareComplexSymbolType(type));
    ...
    else
    {
        typename std::array<uint32_t, N>::iterator firstUnused =
            std::find(symbolArray.begin(), symbolArray.end(),
                      static_cast<uint32_t>(notUsed));
